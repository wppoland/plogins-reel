...
2. **Run the scaffold script** — replaces all tokens and renames `reel.php → <slug>.php`
   ...
   |---|---|---|
   | `reel` | lowercase slug = text-domain = i18n domain | `restock` |
   | `Reel` | PSR-4 PHP namespace | `Restock` |
   | `REEL` (in `define()`) | uppercased namespace | `RESTOCK` |
   | `reel_` | option/meta prefix (slug, dashes→underscores) | `restock_` |
   | `Reel` / `Hover zoom, lightbox and featured video for the WooCommerce product gallery — reserved space, no jQuery` / `Hover zoom, lightbox and featured video for the WooCommerce product gallery — reserved space, no jQuery` | name + descriptions | … |
...
- **Bootstrap** (`reel.php`): PHP/WC guards, HPOS + cart-blocks compat, `plugins_loaded`
  boot, `do_action('reel/booted')` (the hook a PRO companion extends).
